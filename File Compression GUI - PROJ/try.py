...
class Window(CTk):
    ...
    def openfile(self):
            fpath = self.openFile()
            spath = self.fileSave()
            customtkinter.CTk().splitlist(fpath)
            print(spath)
   
            with open(fpath, 'rb') as f, open(spath + ".xz", 'wb') as out:
                out.write(xz.compress(bytes(f.read())))

            label = customtkinter.CTkLabel(master=self, text="file is saved at "+ spath+".xz", font=("rockwell", 12))
